...
fn can_transition_tls_state(from TlsState, to TlsState) bool {
	...
	match from {
		.ts_closed { return to == .ts_init }
		.ts_init { return to == .ts_client_hello }
		.ts_client_hello { return to == .ts_server_hello || to == .ts_server_hello_2 }
		.ts_server_hello { return to == .ts_client_hello || to == .ts_encrypted_extensions || to == .ts_closing }
		.ts_server_hello_2 { return to == .ts_encrypted_extensions || to == .ts_closing }
		.ts_encrypted_extensions { return to == .ts_server_certificate_request || to == .ts_server_finished }
		.ts_server_certificate_request { return to == .ts_server_certificate || to == .ts_server_certificate_verify }
		.ts_server_certificate { return to == .ts_server_certificate_verify }
		.ts_server_certificate_verify { return to == .ts_server_finished }
		.ts_server_finished { return to == .ts_client_certificate || to == .ts_client_finished }
		.ts_client_certificate { return to == .ts_client_certificate_verify || to == .ts_client_finished }
		.ts_client_certificate_verify { return to == .ts_client_finished }
		.ts_client_finished { return to == .ts_connected }
		.ts_connected { return to == .ts_application_data }
		.ts_application_data { return to == .ts_key_update || to == .ts_closing }
		.ts_key_update { return to == .ts_application_data }
		.ts_closing { return to == .ts_closed }
		.ts_early_data { return to == .ts_endof_early_data || to == .ts_server_hello }
		.ts_endof_early_data { return to == .ts_server_finished }
		.ts_change_cipher_spec { return to == .ts_server_hello || to == .ts_encrypted_extensions }
	}
}
