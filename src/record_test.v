// Copyright © 2025 blackshirt.
// Use of this source code is governed by an MIT license
// that can be found in the LICENSE file.
//
// Testing for various TLS 1.3 record basic functionalities
module tls13

// Test for ClientHello handshake record where the material was taken from
// RFC 8448 of Example Handshake Traces for TLS 1.3
fn test_clienthello_record_from_rfc8448() ! {
	// 3.  Simple 1-RTT Handshake
	// See at https://datatracker.ietf.org/doc/html/rfc8448#section-3
	// complete record (201 octets)
	clienthello_record := [u8(0x16), 0x03, 0x01, 0x00, 0xc4, u8(0x01), ]

	rec := parse_rec(clienthello_record)!
	// Its a handshake type
	assert rec.ctype == .handshake
	assert rec.version == .tls10 // 0300

	hsk := parse_hsk(rec.fragment)!
	// its should client_hello msg
	assert hsk.tipe == .client_hello

	ch := parse_chello(hsk.payload)!
	assert ch.version == .tls12
	assert ch.random.len == 32
	assert ch.sessid.len == 0

	// [TLS_AES128GCM_SHA256, TLS_CHACHA20POLY1305_SHA256, TLS_AES256GCM_SHA384]
	assert ch.csuites.len == 3
	assert ch.csuites[0] == .tls_aes128gcm_sha256
	assert ch.csuites[1] == .tls_chacha20poly1305_sha256
	assert ch.csuites[2] == .tls_aes256gcm_sha384

	assert ch.cmeths.len == 1
	assert ch.cmeths[0] == 0

	assert ch.xslist.len == 9

	// serializes it back
	ch_payload := pack_chello(ch)!
	hsk_back := Handshake{
		tipe:    .client_hello
		payload: ch_payload
	}

	// make record
	rec_back := TlsRecord{
		ctype:    .handshake
		version:  rec.version
		fragment: pack_hsk(hsk_back)!
	}
	// encodes the record
	rec_bytes := pack_rec(rec_back)!
	assert rec_bytes == clienthello_record
}

// Test for Certificate handshake message where the material was taken from
// RFC 8448 of Example Handshake Traces for TLS 1.3
fn test_certificate_hskmsg_from_rfc8448() ! {
	// See RFC 448  [Page 7]
	// Certificate (445 octets)
	certificate_msg := [u8(0x0b), ]

	hsk := parse_hsk(certificate_msg)!
	assert hsk.tipe == .certificate

	cert := parse_cert(hsk.payload)!
	assert cert.context.len == 0
	// // single CertificateEntry, we have no support to parse ASN.1 DER certificate
	assert cert.celist.len == 1
	assert cert.celist[0].opaque.len == 432
	assert cert.celist[0].xslist.len == 0

	// encodes it back
	hsk_back := Handshake{
		tipe:    .certificate
		payload: pack_cert(cert)!
	}

	assert pack_hsk(hsk_back)! == certificate_msg
}

// Test for ServerHello handshake record where the material was taken from
// RFC 8448 of Example Handshake Traces for TLS 1.3
fn test_serverhello_record_from_rfc8448() ! {
	// complete record (95 octets)
	// See RFC 8448 [Page 7]
	serverhello_record := [u8(0x16), 0x03, 0x03, 0x00, 0x5a, u8(0x02), 0x00, 0x00, 0x56, 0x03,
		0x03, 0xa6, 0xaf, 0x06, 0xa4, 0x12, 0x18, 0x60, 0xdc, 0x5e, 0x6e, 0x60, 0x24, 0x9c, 0xd3,
		0x4c, 0x95, 0x93, 0x0c, 0x8a, 0xc5, 0xcb, 0x14, 0x34, 0xda, 0xc1, 0x55, 0x77, 0x2e, 0xd3,
		0xe2, 0x69, 0x28, 0x00, 0x13, 0x01, 0x00, 0x00, 0x2e, 0x00, 0x33, 0x00, 0x24, 0x00, 0x1d,
		0x00, 0x20, 0xc9, 0x82, 0x88, 0x76, 0x11, 0x20, 0x95, 0xfe, 0x66, 0x76, 0x2b, 0xdb, 0xf7,
		0xc6, 0x72, 0xe1, 0x56, 0xd6, 0xcc, 0x25, 0x3b, 0x83, 0x3d, 0xf1, 0xdd, 0x69, 0xb1, 0xb0,
		0x4e, 0x75, 0x1f, 0x0f, 0x00, 0x2b, 0x00, 0x02, 0x03, 0x04]

	rec := parse_rec(serverhello_record)!
	assert rec.ctype == .handshake
	assert rec.version == .tls12

	hsk := parse_hsk(rec.fragment)!
	assert hsk.tipe == .server_hello

	sh := parse_shello(hsk.payload)!

	assert sh.version == .tls12
	assert sh.csuite == .tls_aes128gcm_sha256

	assert sh.xslist.len == 2
	assert sh.xslist[0].tipe == .key_share
	assert sh.xslist[1].tipe == .supported_versions

	// encodes it back
	hsk_back := Handshake{
		tipe:    .server_hello
		payload: pack_shello(sh)!
	}

	rec_back := TlsRecord{
		ctype:    .handshake
		version:  rec.version
		fragment: pack_hsk(hsk_back)!
	}
	assert pack_rec(rec_back)! == serverhello_record
}

// Test for Finished handshake message where the material was taken from
// RFC 8448 of Example Handshake Traces for TLS 1.3
fn test_finished_hskmsg_from_rfc8448() ! {
	// Finished (36 octets):  14 00 00 20 a8 ec 43 6d 67 76 34 ae 52 5a
	//     c1 fc eb e1 1a 03 9e c1 76 94 fa c6 e9 85 27 b6 42 f2 ed d5 ce
	//     61
	client_finished_record := [u8(0x14), 0x00, 0x00, 0x20, 0xa8, 0xec, 0x43, 0x6d, 0x67, 0x76,
		0x34, 0xae, 0x52, 0x5a, 0xc1, 0xfc, 0xeb, 0xe1, 0x1a, 0x03, 0x9e, 0xc1, 0x76, 0x94, 0xfa,
		0xc6, 0xe9, 0x85, 0x27, 0xb6, 0x42, 0xf2, 0xed, 0xd5, 0xce, 0x61]

	hsk := parse_hsk(client_finished_record)!
	assert hsk.tipe == .finished
	fin := finished_from_hsk(hsk)!

	assert fin.verify_data == [u8(0xa8), 0xec, 0x43, 0x6d, 0x67, 0x76, 0x34, 0xae, 0x52, 0x5a,
		0xc1, 0xfc, 0xeb, 0xe1, 0x1a, 0x03, 0x9e, 0xc1, 0x76, 0x94, 0xfa, 0xc6, 0xe9, 0x85, 0x27,
		0xb6, 0x42, 0xf2, 0xed, 0xd5, 0xce, 0x61]
}

fn test_find_ctntype_offset() {
	data := [u8(0), 8, 117, 139, 70, 22, 0, 0, 0, 0]
	pos := find_ctype_offset(data)
	assert pos == 5
	out := parse_innertext(data)!
	assert u8(out.ctype) == u8(22) // .handshake
	assert out.zeros.len == 4
	assert out.content == [u8(0), 8, 117, 139, 70]
}

/*
import encoding.binary
import crypto.rand
import blackshirt.ecdhe

// rfc 8448
const client_finished_record = [u8(0x17), 0x03, 0x03, 0x00, 0x35, 0x75, 0xec, 0x4d, 0xc2, 0x38,
	0xcc, 0xe6, 0x0b, 0x29, 0x80, 0x44, 0xa7, 0x1e, 0x21, 0x9c, 0x56, 0xcc, 0x77, 0xb0, 0x51, 0x7f,
	0xe9, 0xb9, 0x3c, 0x7a, 0x4b, 0xfc, 0x44, 0xd8, 0x7f, 0x38, 0xf8, 0x03, 0x38, 0xac, 0x98, 0xfc,
	0x46, 0xde, 0xb3, 0x84, 0xbd, 0x1c, 0xae, 0xac, 0xab, 0x68, 0x67, 0xd7, 0x26, 0xc4, 0x05, 0x46]

fn test_unpack_tls13_finished_record() ! {
	rec := pack_rec(client_finished_record)!
	assert rec.length == 53
	assert rec.ctn_type == .application_data
}

fn test_from_openssl_output_read() ! {
	data := [u8(0x16), ]
	rec := pack_rec(data)!
	// dump(rec)
}

fn test_session_init() ! {
	mut exchanger := ecdhe.new_exchanger(ecdhe.Curve.x25519)!
	// build needed random bytes
	privkey := exchanger.generate_private_key()!
	pubkey := exchanger.public_key(privkey)!

	random := rand.read(32)!
	sessid := []u8{}
	ciphersuites := [CipherSuite.tls_chacha20_poly1305_sha256]

	mut exts := []Extension{}
	// server_name extension
	srvname := new_server_name('localhost')!
	srvname_list := ServerNameList([srvname])
	srvname_ext := srvname_list.pack_to_extension()!
	exts.append(srvname_ext)

	// SupportedVersions extension
	spv := ClientSpV{
		versions: [tls_v13]
	}
	spv_ext := SupportedVersions(spv).pack_to_extension()!
	exts.append(spv_ext)

	// NamedGroupList
	ngl := NamedGroupList([NamedGroup.x25519])
	ngl_ext := ngl.pack_to_extension()!
	exts.append(ngl_ext)

	// signaturescheme
	signs_list := SignatureSchemeList([SignatureScheme.ed25519])
	signs_ext := signs_list.pack_to_extension()!
	exts.append(signs_ext)

	// KeyShare extension
	ke_entry0 := KeyShareEntry{
		group:        .x25519
		key_exchange: pubkey.bytes()!
	}

	ks := KeyShareExtension{
		msg_type:      .client_hello
		is_hrr:        false
		client_shares: [ke_entry0]
	}
	ks_ext := ks.pack_to_extension()!
	exts.append(ks_ext)
	// TODO: add another supported extension
	ch := ClientHello{
		random:            random
		legacy_session_id: sessid
		cipher_suites:     ciphersuites
		extensions:        exts
	}

	hsk := HandshakePayload(ch).pack_to_handshake()!
	// dump(hsk)
	pxt := TLSPlaintext.from_handshake(hsk)!
	// dump(pxt)

	pxt_obj := pxt.pack()!

	pxt_unpacked := TLSPlaintext.unpack(pxt_obj)!
	assert pxt == pxt_unpacked
	assert pxt_unpacked.ctn_type == .handshake
	assert pxt_unpacked.legacy_version == tls_v12

	hsk_unpacked := Handshake.unpack(pxt_unpacked.fragment)!
	assert hsk_unpacked.msg_type == .client_hello

	ch_unpacked := ClientHello.unpack(hsk_unpacked.payload)!
	assert ch_unpacked == ch
}

fn test_decrypted_data_from_handshake() {
	enc_record := [u8(233), 85, 239, 170, 3, 176, 70, 17, 90, 239, 247, 248, 245, 63, 222, 40,
		138, 128, 207, 31, 251, 196, 51]
	client_hwkey := [u8(238), 106, 116, 243, 11, 38, 234, 25, 255, 244, 246, 236, 67, 226, 151,
		139, 138, 236, 51, 254, 245, 119, 202, 186, 55, 204, 254, 223, 163, 149, 42, 244]
	client_hwiv := [u8(103), 160, 169, 56, 160, 230, 106, 13, 105, 82, 149, 5]
	server_hwkey := [u8(234), 164, 3, 81, 193, 130, 74, 52, 237, 133, 44, 180, 69, 28, 41, 81,
		18, 193, 7, 199, 68, 88, 66, 114, 106, 44, 165, 124, 126, 60, 148, 167]
	server_hwiv := [u8(24), 147, 74, 228, 50, 185, 46, 107, 41, 28, 17, 64]

	cxt := TLSCiphertext{
		opaque_type:    .application_data
		legacy_version: tls_v12
		length:         23
		enc_record:     enc_record
	}
	mut rc := new_record_layer(.tls_chacha20_poly1305_sha256)!
	snonce := rc.build_read_nonce(server_hwiv)
	cnonce := rc.build_read_nonce(client_hwiv)

	mut add := cxt.opaque_type.pack()!
	add << cxt.legacy_version.pack()!
	mut length := []u8{len: 2}
	binary.big_endian_put_u16(mut length, u16(cxt.length))
	add << length
	cxt_len := cxt.enc_record.len
	ciphertext := cxt.enc_record[0..cxt_len - rc.cipher.tag_size()].clone()
	plain, tag := rc.cipher.decrypt(server_hwkey, cnonce, add, ciphertext)!

	innertext := TLSInnerPlaintext.unpack(plain)!
	pxt := innertext.to_plaintext()!

	// TODO: need fix
	// dump(pxt)
}



fn test_ee_encrypt_decrypt() {
	ee_data := [u8(0x08), 0x00, 0x00, 0x02, 0x00, 0x00]
	hsk := Handshake.unpack(ee_data)!
	assert hsk.msg_type == .encrypted_extensions
	assert hsk.length == 2
	ee := EncryptedExtensions.unpack(hsk.payload)!
	assert ee.extensions.len == 0

	client_hwkey := [u8(238), 106, 116, 243, 11, 38, 234, 25, 255, 244, 246, 236, 67, 226, 151,
		139, 138, 236, 51, 254, 245, 119, 202, 186, 55, 204, 254, 223, 163, 149, 42, 244]
	client_hwiv := [u8(103), 160, 169, 56, 160, 230, 106, 13, 105, 82, 149, 5]
	server_hwkey := [u8(234), 164, 3, 81, 193, 130, 74, 52, 237, 133, 44, 180, 69, 28, 41, 81,
		18, 193, 7, 199, 68, 88, 66, 114, 106, 44, 165, 124, 126, 60, 148, 167]
	server_hwiv := [u8(24), 147, 74, 228, 50, 185, 46, 107, 41, 28, 17, 64]

	mut rc := new_record_layer(.tls_chacha20_poly1305_sha256)!
	snonce := rc.build_read_nonce(server_hwiv)
	cnonce := rc.build_read_nonce(client_hwiv)

	pxt := TLSPlaintext.from_handshake(hsk)!
	inner := pxt.to_innerplaintext_with_padmode(.nopad)!
	plaintext := inner.pack()!
	assert plaintext.len == 7
	length := plaintext.len + rc.cipher.tag_size()

	add := rc.make_additional_data(ContentType.application_data, tls_v12, length)!
	ciphertext, tag := rc.cipher.encrypt(server_hwkey, snonce, add, plaintext)!

	mut enc_record := []u8{}
	enc_record << ciphertext
	enc_record << tag
	trec := TLSCiphertext{
		opaque_type:    .application_data
		legacy_version: tls_v12
		length:         23
		enc_record:     enc_record
	}

	txrec := rc.encrypt(pxt, server_hwkey, snonce)!
	assert trec == txrec
	cxt := trec.enc_record[0..trec.enc_record.len - rc.cipher.tag_size()]
	assert cxt == ciphertext
	dcc, mcc := rc.cipher.decrypt(server_hwkey, snonce, add, cxt)!
	dec_trec := rc.decrypt(trec, server_hwkey, server_hwiv)!

	assert dcc == plaintext
	assert dec_trec == pxt
	assert tag == mcc

	dcc_inn := TLSInnerPlaintext.unpack(dcc)!
	exx := dcc_inn.to_plaintext()!

	eex := Handshake.unpack(exx.fragment)!
	assert eex.msg_type == .encrypted_extensions
	assert eex.length == 2
}

fn test_certificate_msg_from_session() ! {
	cert_msg := [u8(0x0b), ]

	cert := Handshake.unpack(cert_msg)!
	certback := cert.pack()!
	assert certback.len == 0x01b0 // 132

	assert cert_msg == certback
}
*/
